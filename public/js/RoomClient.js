...
class RoomClient {
    ...
    */
    ...
    initSockets() {
        ...
        this.socket.on(
            ...
            function (data) {
                ...
                document.getElementById('participantsCountMenu').innerHTML = '(' + participantsCount + ')';
            }.bind(this),
        );
    ...
    setVideoAvatarImgName(elemId, peer_name, profileImageUrl = null) {
        let elem = this.getId(elemId);
        let avatarImgSize = 32;
        elem.setAttribute(
            'src',
            profileImageUrl
                ? profileImageUrl
    ...
    setIsAudio(peer_id, status) {
        ...
        var cotrolSelector = controlDiv ? controlDiv.querySelector('#startAudioButton') : false;
        if (controlDiv && cotrolSelector) {
            if (status) {
                cotrolSelector.style.display = 'flex';
            } else {
                cotrolSelector.style.display = 'none';
            }
        }
    }
    ...
    sendMessage() {
        ...
        const profileImageUrl = getQueryString(window.location.search, 'profileImageUrl');
        let data = {
            peer_name: this.peer_name,
            peer_msg: peer_msg,
            peer_img: profileImageUrl,
        };
        console.log('Send message:', data);
        this.socket.emit('message', data);
        this.setMsgAvatar('right', this.peer_name, profileImageUrl);
    ...
    showMessage(data) {
        ...
        this.setMsgAvatar('left', data.peer_name, data.peer_img);
        this.appendMessage('left', this.leftMsgAvatar, data.peer_name, data.peer_msg);
        this.sound('message', true);
    }

    setMsgAvatar(avatar, peerName, profileImageUrl = null) {
        let avatarImg = profileImageUrl
            ? profileImageUrl
            : cfg.msgAvatar + '?name=' + peerName + '&size=32' + '&background=random&rounded=true';
        ...
        if (isImageURL(this.incomingFileInfo.fileName)) {
            ...
            reader.onload = (e) => {
                Swal.fire({
                    ...
                    imageAlt: 'talgiving-file-img-download',
                    showDenyButton: true,
