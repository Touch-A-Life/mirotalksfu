...
class RoomClient {
    ...
    socket
      ...
      .then(
        async function (room) {
          ...
          console.log(
            "07 ----> Get Router Rtp Capabilities codecs: ",
  ...
  initSockets() {
    ...
    this.socket.on(
      ...
      function (data) {
        ...
        document.getElementById("participantsCountMenu").innerHTML =
          "(" + participantsCount + ")";
      }.bind(this)
    );
  ...
  async setVideoOff(peer_info, remotePeer = false) {
    ...
    s.id = peer_info.peer_id + "__enableSpeaker";
    s.className = "fas fa-phone-volume";
    s.style.cssText = "display: flex;left: 25px";
  ...
  youTubeAction(data) {
    ...
    switch (action) {
      case "open":
        this.userLog(
          "info",
          `${peer_name} <i class="fab fa-youtube"></i> opened the YouTube video`,
          "top-end"
        );
        this.openYouTube(data);
        break;
      case "close":
        this.userLog(
          "info",
          `${peer_name} <i class="fab fa-youtube"></i> closed the YouTube video`,
          "top-end"
        );
        this.closeYouTube();
        break;
