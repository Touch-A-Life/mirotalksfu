...
app.get("/join/:room", async (req, res) => {
  ...
  const checkRoom = await ChatRoomModel.findById(room);
  if (!checkRoom || !checkRoom._id) {
    return res.sendFile(path.join(__dirname, "../../", "public/view/404.html"));
  }
...
app.post(["/api/v1/meeting"], (req, res) => {
  ...
  if (!api.isAuthorized()) {
    log.debug("TALGiving audio rooms - Unauthorized", {
      header: req.headers,
      body: req.body,
    });
  ...
  log.debug("TALGiving audio rooms - Authorized", {
    header: req.headers,
    body: req.body,
    meeting: meetingURL,
  });
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    if (
      roomList.get(socket.room_id).getPeers().size === 0 &&
      roomList.get(socket.room_id).isLocked()
    ) {
      roomList.get(socket.room_id).setLocked(false);
    }
